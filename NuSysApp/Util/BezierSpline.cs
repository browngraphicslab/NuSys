...
namespace NuSysApp.Util
{
    public static class BezierSpline
    {
        ...
        public static void GetCurveControlPoints(Point2d[] knots,
            out Point2d[] firstControlPoints, out Point2d[] secondControlPoints)
        {
            if (knots == null)
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                if (i < n - 1)
                    secondControlPoints[i] = new Point2d(2 * knots
                        [i + 1].X - x[i + 1], 2 *
                        knots[i + 1].Y - y[i + 1]);
                else
                    secondControlPoints[i] = new Point2d((knots
                        [n].X + x[n - 1]) / 2,
                        (knots[n].Y + y[n - 1]) / 2);
            }
        }
