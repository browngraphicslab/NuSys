...
namespace NuSysApp
{
    public class NuSysRenderer
    {
        ...
        public async Task Init(CanvasAnimatedControl canvas)
        {
            ...
            ActiveCollection = new CollectionRenderItem(vm, null, canvas, true);
            _currentCollection = ActiveCollection;

            _interactionManagers[_currentCollection] = new CollectionInteractionManager(ActiveCollection);
            _interactionManagers[_currentCollection].ItemTapped += OnItemTapped;
            _interactionManagers[_currentCollection].ItemLongPressed += OnItemLongPressed;
            ...
            _elementSelectionRenderItem = new ElementSelectionRenderItem(vm, ActiveCollection, _canvas);
     
            _minimap = new MinimapRenderItem(vm, ActiveCollection, canvas);
        }

        private void OnItemLongPressed(BaseRenderItem element, PointerDeviceType device)
        {
            ...
            if (collection != _currentCollection ) { 
                ...
                _interactionManagers[collection].ItemLongPressed += OnItemLongPressed;
