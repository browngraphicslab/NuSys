...
namespace NuSysApp
{
    public sealed partial class WebDetailView : UserControl
    {
        ...
        public WebDetailView(WebNodeViewModel vm)
        {
            ...
            vm.Controller.LibraryElementController.ContentDataController.ContentDataUpdated += delegate (object sender, string text)
            {
                OnUrlChanged(text);
            };
        }
        ...
        private void XWebView_OnNavigationCompleted(WebView sender, WebViewNavigationCompletedEventArgs args)
        {
            ...
            {
                Back.IsEnabled = true;
                _viewMod.Controller.LibraryElementController?.ContentDataController.SetData(url);
