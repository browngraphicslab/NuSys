...
namespace NuSysApp.Misc.SpeechToTextUI
{
    ...
    public class CorrectionSuggester
    {
        ...
        /// <returns></returns>
        internal List<WordAlternates> GetAlternates(List<string> hypothesesList, string results)
        {
            ...
            List<WordAlternates> output = new List<WordAlternates>();
            for (int i = 0; i < words.Length && i < alternateWordsByIndex.Count; i++)
            {
                output.Add(new WordAlternates(words[i], alternateWordsByIndex[i]));
            }
            return output;
        }
