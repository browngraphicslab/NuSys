...
namespace LDAUser
{
    public class Model
    {
        ...
        private TopicWordPercent[,] _topicWordPercents;
            ...
            {   //switched it from a list of pairs (the one we make) to a list of keyvaluepairs, cuz SCREW JAVA CRAP. SAY HELLO TO CEEEEEE SHARPEEEEE
                ...
                for (int i = 0; i < twords; i++)
                {
                    if (data.localDict.contains((int)wordsProbsList[i].Key))
                    {
                        ...
                        {
                            wordList.Add(word);
                            _topicWordPercents[k,i] = new TopicWordPercent(word, wordsProbsList[i].Value, k, theta[0,k] );
                        }
                    }
                }
            } // end for each topic
        ...
        public TopicWordPercent[,] TopicWordPercents
        {
            get { return _topicWordPercents; }
        }
        ...
        public async Task<bool> saveModel(string modelName)
        {
            ...
            if (twords > 0)
            {
                temp = saveModelTwords(dir + System.IO.Path.DirectorySeparatorChar + modelName + twordsSuffix + ".txt");
                if (!temp)
                    return false;
            }
            return true;
        ...
        public async Task<bool> initNewModel(Option option, List<string> documents ){
            ...
            _topicWordPercents = new TopicWordPercent[K, twords];
    ...
    public sealed class TopicWordPercent
    {
        private string _word;
        private double _wordProb;
        private int _topic;
        private double _topicPercent;
        public TopicWordPercent(string word, double wordProb, int topic, double topicPercent)
        {
            _word = word;
            _wordProb = wordProb;
            _topic = topic;
            _topicPercent = topicPercent;
        ...
        {
            get { return _wordProb; }
        }
        public int Topic
        {
            get { return _topic; }
        }
        public double TopicPercent
        {
            get { return _topicPercent; }
        }
    }
}
