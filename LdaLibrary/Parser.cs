...
namespace LdaLibrary
{
    public class Parser
    {
        ...
        public async static Task<Dictionary<string,string>> GetAllWikiContent(List<string> topics)
        {
            ...
            foreach (string t in topics)
            {
                ...
                List<string> topTopicPage = await GetTopPages(formattedTopic, 1);
                if (allText.ContainsKey(topTopicPage[0]))
                {
                    continue;
                }
                string s = await GetWikiContent(topTopicPage[0]);
                string introText = GetParsedContent(GetIntroText(s)).ToLower();
                allText.Add(topTopicPage[0],GetParsedContent(s));
                /*List<string> links = await GetWikiLink(topTopicPage[0], "https://en.wikipedia.org/w/api.php?format=json&action=query&titles=" + topTopicPage[0] + "&prop=links&pllimit=max");
                if (links.Count > 0)
                {
                    foreach (string link in links)
                    {
                        ...
                        {
                            continue; // this is the case in which there is no intro paragraph
                        }
        ...
        public async static Task<List<string>> GetTopPages(string topic, int numPages)
        {
            ...
            {
                page = await client.GetStringAsync(url).ConfigureAwait(false);
            }
        ...
        public static Dictionary<string,double> GetTopicCount(Dictionary<string,string> allTopics, List<string> allWords)
        {
        ...
        public static string GetIntroText(string content)
        {
        ...
        public static string GetParsedContent(string content)
        {
        ...
        public static List<string> GetWordPairs(List<string> input)
        {
        ...
        public async static Task<List<string>> GetWikiLink(string page, string url)
        {
            string linkPage, id;
            ...
            using (HttpClient client = new HttpClient())
            {
                linkPage = await client.GetStringAsync(url).ConfigureAwait(false);
