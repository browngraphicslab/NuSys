    ...
    public class HtmlImporter
    {
        ...
        public async Task<IEnumerable<DataHolder>> Run(Uri url)
        {
            ...
            foreach (var paragraphs in _paragraphCollections)
            {
                ...
                var content = new TextDataHolder(text, title ?? "") {links = _citationUrlCollections.First()};
                _citationUrlCollections.RemoveAt(0);
                models.Add(content);
            }
            return models;
        ...
        /// </summary>
        private readonly HashSet<HashSet<HtmlNode>> _paragraphCollections = new HashSet<HashSet<HtmlNode>>();

        /// <summary>
        /// This is the id of each citation in the same order as each paragraph so that we can find the links later
        /// </summary>
        private readonly HashSet<List<string>> _citationCollections = new HashSet<List<string>>();
        /// <summary>
        /// These are the links to each document that is being cited
        /// </summary>
        private readonly List<List<string>> _citationUrlCollections = new List<List<string>>();
        ...
        /// <returns></returns>
        private async Task RecursiveAdd(HtmlNode node, HashSet<DataHolder> models)
        {
