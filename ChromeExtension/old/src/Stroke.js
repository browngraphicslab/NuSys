...
var Stroke = (function () {
    ...
    Stroke.prototype.resample = function(numSamples) {
        ...
        this.points = newSamples.slice(0);
            ...
            /*
            var S = this.points.length/(samples - 1) - 0.001;
            var D = 0.0, d; // D is the distance accumulator of consecutive points, when D < S
            ...
            for (i = 1; i < ps.length; i++)
            {
                ...
                var v0 = Vector2.fromPoint(ps[i-1]);
                var v1 = Vector2.fromPoint(ps[i]);
                d = v0.distanceTo(v1);
                ...
                if (D + d >= S)
                {
                    c = c + 1;
                    var q = {};
                    q.x = ps[i - 1].x + ((S - D)/d)*(ps[i].x - ps[i - 1].x);
                    q.y = ps[i - 1].y + ((S - D)/d)*(ps[i].y - ps[i - 1].y);
                    newPoints.push(q);
                    ps.splice(i, 0, q);
